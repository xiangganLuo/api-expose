...
import com.api.expose.framework.common.biz.system.permission.dto.DeptDataPermissionRespDTO;
...
import com.api.expose.system.controller.admin.user.vo.user.UserRoleImportExcelVO;
import com.api.expose.system.controller.admin.user.vo.user.UserRoleImportRespVO;
import com.api.expose.system.dal.dataobject.permission.MenuDO;
import com.api.expose.system.dal.dataobject.permission.RoleDO;
import com.api.expose.system.dal.dataobject.permission.RoleMenuDO;
import com.api.expose.system.dal.dataobject.permission.UserRoleDO;
import com.api.expose.system.dal.mysql.permission.RoleMenuMapper;
import com.api.expose.system.dal.mysql.permission.UserRoleMapper;
import com.api.expose.system.dal.redis.RedisKeyConstants;
import com.api.expose.system.enums.permission.DataScopeEnum;
...
public class PermissionServiceImpl implements PermissionService {
    ...
    public DeptDataPermissionRespDTO getDeptDataPermission(Long userId) {
        ...
        Supplier<Set<Long>> userDeptIds = Suppliers.memoize(() -> userService.getUser(userId).getDeptIds());
        // 遍历每个角色，计算
        for (RoleDO role : roles) {
            ...
            if (Objects.equals(role.getDataScope(), DataScopeEnum.DEPT_AND_CHILD.getScope())) {
                if (userDeptIds.get() != null) {
                    // 获取用户所在的每个部门及其子部门
                    for (Long deptId : Objects.requireNonNull(userDeptIds.get())) {
    ...
    @Override
    public UserRoleImportRespVO importUserRoleList(List<UserRoleImportExcelVO> importUsers) {
        int successCount = 0;
        List<Long> failureUserIds = new ArrayList<>();
        for (UserRoleImportExcelVO vo : importUsers) {
            try {
                if (vo.getUserId() == null || vo.getRoleIds() == null) {
                    failureUserIds.add(vo.getUserId());
                    continue;
                }
                // 解析角色ID
                String[] roleIdArr = vo.getRoleIds().split(",");
                Set<Long> roleIdSet = new HashSet<>();
                for (String roleIdStr : roleIdArr) {
                    try {
                        roleIdSet.add(Long.valueOf(roleIdStr.trim()));
                ...
                // 是否支持更新逻辑（可选）
                // 这里可以根据 isUpdateSupport 判断是否先清空原有角色再绑定
                // 调用服务绑定
                assignUserRole(vo.getUserId(), roleIdSet);
                successCount++;
            } catch (Exception e) {
                failureUserIds.add(vo.getUserId());
            }
        }
        UserRoleImportRespVO respVO = new UserRoleImportRespVO();
        respVO.setCreateSuccessCount(successCount);
        respVO.setFailureUserIds(failureUserIds);
        return respVO;
    }
