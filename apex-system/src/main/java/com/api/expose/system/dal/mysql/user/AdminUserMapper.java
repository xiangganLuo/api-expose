package com.api.expose.system.dal.mysql.user;
...
public interface AdminUserMapper extends BaseMapperX<AdminUserDO> {
    ...
    default List<AdminUserDO> selectListByDeptIds(Collection<Long> deptIds) {
        return selectList(AdminUserDO::getDeptIds, deptIds);
    }

    /**
     * 根据业务表查询实际使用的用户
     * 连表查询业务表中存在的用户，去重且只返回启用的用户
     * 
     * @param tableName 业务表名
     * @return 用户列表
     */
    List<AdminUserDO> selectUsersByTable(@Param("tableName") String tableName);
