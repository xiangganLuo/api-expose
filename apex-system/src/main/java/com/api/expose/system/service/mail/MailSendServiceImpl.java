package com.api.expose.system.service.mail;
...
import com.api.expose.system.mq.message.mail.MailSendMessage;
import com.api.expose.system.mq.producer.mail.MailProducer;
import com.api.expose.system.service.member.MemberService;
...
public class MailSendServiceImpl implements MailSendService {
    ...
    public Long sendSingleMailToAdmin(String mail, Long userId,
        ...
        return sendSingleMailToAdmin(mail, userId, templateCode, templateParams, null);
    }
    ...
    public Long sendSingleMailToAdmin(String mail, Long userId,
        ...
        // 执行发送
        return sendSingleMail(mail, userId, UserTypeEnum.ADMIN.getValue(), templateCode, templateParams, attachments);
    }

    @Override
    public Long sendSingleMailToMember(String mail, Long userId,
                                       String templateCode, Map<String, Object> templateParams) {
        return sendSingleMailToMember(mail, userId, templateCode, templateParams, null);
    }
    ...
    public Long sendSingleMailToMember(String mail, Long userId,
        ...
        // 执行发送
        return sendSingleMail(mail, userId, UserTypeEnum.MEMBER.getValue(), templateCode, templateParams, attachments);
    }

    @Override
    public Long sendSingleMail(String mail, Long userId, Integer userType,
                               String templateCode, Map<String, Object> templateParams) {
        return sendSingleMail(mail, userId, userType, templateCode, templateParams, null);
    }
    ...
    public Long sendSingleMail(String mail, Long userId, Integer userType,
        ...
        // 处理附件
        List<MailSendMessage.MailAttachment> attachmentList = processAttachments(attachments);
        ...
        if (isSend) {
            mailProducer.sendMailSendMessage(sendLogId, mail, account.getId(),
                    template.getNickname(), title, content, attachmentList);
        }
        return sendLogId;
    }
    ...
    public void doSendMail(MailSendMessage message) {
        ...
        try {
            ...
            if (message.getAttachments() != null && !message.getAttachments().isEmpty()) {
                // 带附件发送
                List<File> attachmentFiles = prepareAttachmentFiles(message.getAttachments());
                messageId = MailUtil.send(mailAccount, message.getMail(),
                        message.getTitle(), message.getContent(), true, attachmentFiles.toArray(new File[0]));
            } else {
                // 普通发送
                messageId = MailUtil.send(mailAccount, message.getMail(),
     ...
     * 处理附件上传和存储
     */
    private List<MailSendMessage.MailAttachment> processAttachments(List<MultipartFile> attachments) {
        if (attachments == null || attachments.isEmpty()) {
            return null;
        }

        List<MailSendMessage.MailAttachment> result = new ArrayList<>();
        for (MultipartFile file : attachments) {
            ...
            try {
                ...
                String tempFilePath = tempDir + File.separator + "mail_attachment_" + System.currentTimeMillis() + "_" + filename;
                ...
                // 创建附件信息
                MailSendMessage.MailAttachment attachment = new MailSendMessage.MailAttachment();
                attachment.setFilename(filename);
                attachment.setFilePath(tempFilePath);
                attachment.setContentType(file.getContentType());
                attachment.setFileSize(file.getSize());

                result.add(attachment);
            } catch (Exception e) {
                log.error("处理邮件附件失败: {}", file.getOriginalFilename(), e);
                // 可以选择抛出异常或跳过该附件
            }
        }
        return result;
     ...
     * 准备附件文件用于发送
     */
    private List<File> prepareAttachmentFiles(List<MailSendMessage.MailAttachment> attachments) {
        List<File> files = new ArrayList<>();
        for (MailSendMessage.MailAttachment attachment : attachments) {
