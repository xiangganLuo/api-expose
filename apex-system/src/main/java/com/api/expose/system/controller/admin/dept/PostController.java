...
import com.api.expose.system.controller.admin.dept.vo.post.*;
...
public class PostController {
    ...
    @Operation(summary = "根据业务模块获取岗位列表", description = "获取业务表中实际使用的岗位")
    @Parameter(name = "moduleCode", description = "业务模块编码", required = true, example = "RECRUIT_DEMAND")
    public CommonResult<List<PostSimpleRespVO>> getPostListByBusinessModule(@RequestParam("moduleCode") String moduleCode) {
        List<PostDO> list = postService.getPostsByBusinessModule(moduleCode);
        return success(BeanUtils.toBean(list, PostSimpleRespVO.class));
    }
