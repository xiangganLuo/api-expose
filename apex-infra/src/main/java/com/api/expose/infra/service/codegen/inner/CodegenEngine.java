package com.api.expose.infra.service.codegen.inner;
...
import com.api.expose.framework.apilog.core.enums.OperateTypeEnum;
import com.api.expose.framework.common.exception.util.ServiceExceptionUtil;
...
import com.api.expose.framework.common.util.date.DateUtils;
import com.api.expose.framework.common.util.date.LocalDateTimeUtils;
import com.api.expose.framework.common.util.object.BeanUtils;
import com.api.expose.framework.common.util.object.ObjectUtils;
...
import com.api.expose.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.api.expose.infra.dal.dataobject.codegen.CodegenTableDO;
import com.api.expose.infra.enums.codegen.CodegenFrontTypeEnum;
import com.api.expose.infra.enums.codegen.CodegenSceneEnum;
import com.api.expose.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.api.expose.infra.enums.codegen.CodegenVOTypeEnum;
import com.api.expose.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 apex-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 apex-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.api.expose.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/apex-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/apex-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "apex-${table.moduleName}/" + // 顶级模块
                "apex-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "apex-${table.moduleName}/" + // 顶级模块
                "apex-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "apex-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "apex-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "apex-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
