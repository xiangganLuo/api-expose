...
const handleFileSuccess: UploadProps['onSuccess'] = (res: any): void => {
  ...
  // 删除自身（当使用后端直传时，el-upload会保留临时项）
  const tempIndex = fileList.value.findIndex((item: any) => item?.response?.data === (res as any)?.data)
  if (tempIndex > -1) {
    fileList.value.splice(tempIndex, 1)
  }
  const uploadedUrl: string = typeof res === 'string' ? res : res?.data
  if (!uploadedUrl) {
    return
  }
  uploadList.value.push({ name: uploadedUrl, url: uploadedUrl })
